...
export default function CheckoutPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.isPreOrder && !formData.scheduledTime) {
      alert('Mohon pilih waktu untuk pre-order');
      return;
    }
    ...
    if (formData.deliveryType === 'PICKUP' && !formData.name) {
      alert('Mohon lengkapi nama untuk pickup');
      return;
    }
    ...
    try {
      ...
      if (invalidProducts.length > 0) {
        ...
        alert(`Produk berikut tidak tersedia lagi: ${invalidNames}. Silakan hapus dari keranjang.`);
        return;
      }
  ...
  const handleQRISPaymentSuccess = () => {
    ...
    if (currentOrder) {
      ...
      alert('Terjadi kesalahan: Pesanan tidak ditemukan');
    }
  };

  const handleQRISPaymentExpired = () => {
    setShowQRIS(false);
    alert('Pembayaran QRIS telah kadaluarsa. Silakan coba lagi.');
  };
